...
| Sin backend propio de EcoSmart *para datos de usuario* (arquitectura local-first) | Ni `constitution.md` (stack: "Android Nativo… Persistencia Local: Room/SharedPreferences") ni ningún RF/RNF de `spec.md` piden sincronización multi-dispositivo; las únicas dependencias externas declaradas son EcoGPT y la fuente de Puntos Verdes de CABA. **Actualización 2026-09-23**: `backend/` (Python + FastAPI, Render) sí existe como proxy *stateless* de `POST /verificaciones` hacia una API de IA con visión (Gemini free tier desde la ronda 2, RF-073) — no persiste ni centraliza ningún dato de usuario, así que esta decisión se mantiene vigente para `Usuario`/`RegistroVerificacion` (siguen siendo 100% locales en Room). | Backend propio (p. ej. Ktor + PostgreSQL) para centralizar usuarios — rechazado por estar fuera del alcance declarado en `constitution.md` y no ser requerido por ningún RF; ver detalle y riesgo en `research.md` §0/§2.1. |
